...
idioam_lang(){
  source <(curl -sSl 'https://raw.githubusercontent.com/Libe99/mrcyber/main/lang/lang')
  ...
  for arch in $listarq; do
    if ! wget -O $lang_dir/${list_lang[$lg]}/$arch "https://raw.githubusercontent.com/Libe99/mrcyber/main/lang/${list_lang[$lg]}/$arch" &>/dev/null ;then
...
repo_install(){
  link="https://raw.githubusercontent.com/Libe99/mrcyber/main/Repositorios/$VERSION_ID.list"
  case $VERSION_ID in
...
if [[ ! -e $udp_file/UDPserver.sh ]]; then
  mkdir $udp_file
  chmod -R +x $udp_file
  source <(curl -sSL 'https://raw.githubusercontent.com/Libe99/mrcyber/main/module/module')
  idioam_lang
  [[ -e $lang ]] && newlang=$(cat $lang) && [[ ! $newlang = '' ]] && source $udp_file/lang/$newlang/UDPserver
  source /etc/os-release
  check_sistem
	wget -O $udp_file/module 'https://raw.githubusercontent.com/Libe99/mrcyber/main/module/module' &>/dev/null
	chmod +x $udp_file/module
	#source $udp_file/module
	wget -O $udp_file/limitador.sh "https://raw.githubusercontent.com/Libe99/mrcyber/main/limitador.sh" &>/dev/null
...
menu_udp(){
	...
	print_center -ama 'MOD VpsPremium - MR-CYBER'
	msg -bar
